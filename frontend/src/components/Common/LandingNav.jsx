...
import {
  ArrowRight, ChevronDown, ExternalLink, Menu, X, Home,
  BookOpen, FileText,
  HelpCircle, BookMarked, Shield, Mail,
} from 'lucide-react';
...
const RESOURCES_SUBLINKS = [
  ...
  { label: 'Blog', href: '#', icon: FileText, desc: 'Coming soon' },
];
...
export default function LandingNav({ onGetStarted, user, showSectionLinks = false }) {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[rgba(0,0,0,0.8)] backdrop-blur-xl border-b border-[rgba(255,255,255,0.04)]'
          : 'bg-transparent'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-[64px] md:h-[72px]">
          ...
          <div className="hidden md:flex items-center gap-1">
            {showSectionLinks && navLinks.map((link) => (
            ...
            {showSectionLinks && <div className="w-px h-5 mx-1" style={{ background: 'rgba(255,255,255,0.06)' }} />}
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <>
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => navigate('/profile')} className="relative" title="Profile">
