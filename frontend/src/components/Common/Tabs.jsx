...
export default function Tabs({ tabs = [], activeTab, onChange, variant = 'underline', className = '' }) {
  const variantStyles = {
    underline: {
      ...
      activeIndicator: 'absolute bottom-0 left-2 right-2 h-0.5 bg-gradient-to-r from-[#c8c8d0] to-[#dedee4] rounded-full',
    },
    pills: {
      ...
      tab: (active) => `px-4 py-2 text-[12px] font-medium rounded-lg transition-all ${
        active ? 'bg-[rgba(200,200,208,0.15)] text-[#dedee4] shadow-sm' : 'text-[rgba(255,255,255,0.45)] hover:text-[rgba(255,255,255,0.7)]'
      }`,
          ...
          >
            ...
            {tab.count !== undefined && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${
                isActive ? 'bg-[rgba(200,200,208,0.2)] text-[#dedee4]' : 'bg-[rgba(255,255,255,0.06)] text-[rgba(255,255,255,0.4)]'
              }`}>
                {tab.count}
              </span>
            )}
